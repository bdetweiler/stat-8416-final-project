...
Steps:
Download PERMFY2008.xlsx,PERMFY2009.xlsx, PERMFY2010.xlsx,PERMFY2011.xlsx,
PERMFY2012Q4.xlsx,PERMFY2013.xlsx, PERMFY14Q4.xlsx, PERM_DisclosureDataFY15Q4.xlsx,
PERMDisclosureDataFY16.xlsx from department of labor website, under disclosure data. Set working directory as downloaded files directory. Install required libraries before running this script
...
df = filter(dat_all, YEAR>=2015) %>% group_by(JOB_INFO_MAJOR) %>% dplyr::summarise(count_rec = n(), salary = sum(normalized_wage, na.rm = TRUE)) %>%  
...
dat1 = df %>%  dplyr::group_by(JOB_MAJOR) %>%  
...
df1 <- dat_all %>%  dplyr::group_by(FOREIGN_WORKER_INFO_INST) %>% filter(!is.na(FOREIGN_WORKER_INFO_INST)) %>%
  dplyr::summarize(AVG_SALARY = mean(normalized_wage, na.rm = T), APPLICATIONS = n()) %>%
...
geom_bar(stat="identity") +xlab("University of Education")+ylab("High Avg. Salary")  + coord_flip() +
  theme(axis.text.x = element_text(angle=75, vjust=1, size=7), axis.text.y = element_text(angle=15, vjust=1, size=5))
