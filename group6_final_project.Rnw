...
library(knitr)
library(sqldf)
# XXX: Need to get this installed
#library(choroplethr)
...
Here, we look for disparities in wages for domestic and foreign workers if any exist. 
The H-1B visa program allows businesses to hire skilled professionals with at least a bachelor's degree in areas of specialization.

% XXX: This stat seems to come out of nowhere. Do we have a reason for it? Do we have a source to cite?
% From 2000 to 2005 over 50\% of the applicants were from China and India. 
...
We plan on using 27 attributes common across 2008-2016, as well as an additional 14 attributes found only in 2015-2016. These may be useful for limited analysis. 
In this data, we have employer and employee details, job function, salary, university, job city, number of years of experience, country of citizenship, and industry.
...
The first step in preparing data was to select all the relevant columns and fill empty columns wth \texttt{NA}s. 
...
Employment state data is both at abbreviated and expanded level. 
We needed to standardize either as a two digit code or expanded for consistency. For this exercise state names are created at the expanded level for all 50 US states as well as US territories.
...
Another RDS file (PermEmpMapsdat.rds) is created with summarized employer name address, city, state and zip codes order by number or perm applications processed and by their mean salary.
...
This program will collect all the addresses and store them in a temporary file and updat them again based on their index in a main file. This program is designed to run at multiple locations for different index ranges to collect as much address as possible.
...
PERM data is comprehensive for 2015 and 2016, however the data for each year have inconsistencies. We had to make some educated judgements about some categories and ignore many cases with empty values. 
...
Here is a sample of the categorical data:
...
Using these statistics, we can gague an increasing or decreaseing trend in the number of applications processed.
Although there are caps on the annual number of employment based immigrations, an increasing trend or maximum use of employment based immigration would provide insight.
...
fy <- 2007
#jobs.count <- job.stats$tot[which(job.stats$fy > fy)]
visas.count <- visas.by.status$tot[which(visas.by.status$fy > fy)]
visas.year <- visas.by.status$fy[which(visas.by.status$fy > fy)]

#plot(x = visas.year, y = jobs.count, col="blue", ylim = c(0, 3500000))
plot(x = visas.year, visas.count, col="red")

visas.lm <- lm(visas.count ~ visas.year)
#jobs.lm <- lm(jobs.count ~ visas.year)
abline(visas.lm, col="red")
#abline(jobs.lm, col="blue")

# corr.coef <- cov(x = visas.count, y = jobs.count) / (sd(visas.count) * sd(jobs.count))
visas.lm.summary <- summary(visas.lm)
#jobs.lm.summary <- summary(jobs.lm)
visas.lm.pval <- visas.lm.summary$coefficients[2, "Pr(>|t|)"]
#jobs.lm.pval <- jobs.lm.summary$coefficients[2, "Pr(>|t|)"]
#visas.lm.pval
