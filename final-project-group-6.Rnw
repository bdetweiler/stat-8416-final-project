...
Foreign born workers make up 16.7\% of the United States workforce. In 2014, nearly one million foreign nationals became lawful permanent residents in the United States. Of those one million, 140,000 came through visas which are allocated to employment based residency. Where are these workers, and what do the demographics look like? How does their compensation measure up? Here, we use statistical analysis to examine visa application data from the U.S. Department of Labor from 2008 to 2016. We analyze similar reports done in previous years to determine what changes, if any have taken place in the last ten years.
...
currency.format.kable <- function(x, cents = FALSE) { 
  dig <- 0
  if (cents) {
    dig <- 2
  } 
...
With the exception of exporting data to CSV format, the rest of the steps have been combined into a single script, \texttt{csv\_manipulation.R}, to produce a final data set of \Sexpr{digits.format(dim(visas)[1])} records. \cite{GitHub}
...
Once we have all the data available for each year, a final combined version is created by standardizing the columns and structure.

This data set can be used by anyone interested in H-1B and PERM hiring trends to expand on our work.
...
Multiple XLSX files are initially filtered with the selected 41 columns and merged back into one large file containing \Sexpr{digits.format(dim(dat_all)[1])} records.
...
Employment state data was in abbreviated form as well as expanded form, so we needed to standardize to the expanded level for all 50 U.S. states as well as U.S. territories. 
...
The numeric data provided by the H-1B and PERM data sets are mostly in the form of wages, both the wage that the employer is offering and the prevailing wage.\footnote{Prevailing wage is defined as the hourly wage, usual benefits and overtime, paid to the majority of workers, laborers, and mechanics within a particular geographic area.} Also of interest, are the number of workers an applicant is filing for, and the implicit number of applications faceted by status and year.
...
We also look at year-over-year data on employment trends as well as education and other background information.
...
p <- ggplot(visas.by.status.vs.jobs, aes(x = Type, y = Jobs, fill = Type))  +
  geom_bar(stat = "identity") +
...
filename <- "lcas-vs-jobs.png"
ggsave(filename = filename,
       plot = p,
       width = 22, 
       height = 10, 
       units = "cm")
# plotPNG <- readPNG(filename)
# plotRaster <- as.raster(plotPNG)
# grid.raster(plotRaster)
@
...
filename <- "lca-requests-lin-reg.png"
ggsave(filename = filename,
       plot = p,
       width = 22, 
       height = 10, 
...
p <- ggplot(fy2015.programmers, aes(x=jitter(A_MEDIAN), 
                                    y=jitter(normalized_prevailing_wage), 
                                    col=visa_class)) +
  geom_point() +
  geom_abline(intercept = 0, 
              ...
              col = "red", 
              size = 1) +
  labs(title = "2015 Computer Programmer Prevailing Wages", 
       x = "Median Wage", 
       y = "LCA Wage", 
       colour = "Visa Class") +
  theme(title = element_text(size = 20),
...
filename <- "visa-programmer-pw-vs-median.png"
ggsave(filename = filename,
       plot = p,
       width = 22, 
       height = 10, 
...
p <- ggplot(fy2015.programmers, aes(x = jitter(A_MEDIAN), 
                                    y = jitter(normalized_wage), 
                                    col = visa_class)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = "red", size = 1) +
  labs(title = "2015 Computer Programmer Wages", 
       x = "Median Wage", 
       y = "LCA Wage", 
       colour = "Visa Class") +
  theme(title = element_text(size = 20),
...
filename <- "visa-programmer-wage-vs-median.png"
ggsave(filename = filename,
       plot = p,
       width = 22, 
       height = 10, 
...
The 2005 report, which focussed specifically on computer programmers, stated that 84\% of H-1B prevailing wage claims fell below the median U.S. wage when compared with the same occupation in the same state. For the 2015 data, we found that \Sexpr{median.vs.pw.pro}\% of prevailing wage claims fall below the median (see Figure~\ref{fig:visa-programmer-pw-vs-median}). This seems to back up the claims of the CIS report, that employers are making dubious prevailing wage claims in order to hire cheaper programmers. 
...
p <- ggplot(fy2015, aes(x = jitter(A_MEDIAN), 
                        y = jitter(normalized_prevailing_wage), 
                        col = visa_class)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = "red", size = 1) +
  labs(title = "2015 LCA Prevailing Wage vs. Median US Wage", 
       x = "Median Wage", 
       y = "LCA Prevailing Wage", 
       colour = "Visa Class") +
  theme(title = element_text(size = 20),
        axis.title.x = element_text(size = 20),
        axis.title.y = element_text(size = 20))

filename <- "visa-pw-vs-median.png"
ggsave(filename = filename,
       plot = p,
       width = 22, 
       height = 10, 
...
p <- ggplot(fy2015, aes(x=jitter(A_MEDIAN), y=jitter(normalized_wage), col=visa_class)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = "red", size = 1) +
  labs(title = "2015 LCA Wage vs. Median US Wage", 
       x = "Median Wage", 
       y = "LCA Wage", 
       colour = "Visa Class") +
  theme(title = element_text(size = 20),
...
filename <- "visa-wage-vs-median.png"
ggsave(filename = filename,
       plot = p,
       width = 22, 
       height = 10, 
...
Wages in 2005 were \$12,000 below the median on average, and in 2015, we found this had not really changed. In general, however, wages were only \Sexpr{currency.format(wage.delta)} below the median on average. The standard deviation of prevailing wage claims was \Sexpr{currency.format(round(sd(fy2015$normalized_prevailing_wage), 2))}. 
...
Permanent residents (commonly known as green card holders), have shown a steady increase in median salary over the years, and the number of applications has increased as well.

<<perm-med-wage, fig.pos='hbtp', out.width='.45\\linewidth', echo=FALSE, error=FALSE, warning=FALSE, message=FALSE>>=
...
df$MED_SALARY <- currency.format.kable(df$MED_SALARY)

kable(df, col.names = c("Year", "Median Salary", "Applications"), format = 'latex', caption = "PERM Median Pay and Number of Applications by Year", booktabs = TRUE)
@


<<perm-med-wage-eng, fig.cap='Median wage for PERM workers in Engineering Fields by State', fig.width=8, fig.height=7, fig.align='center', fig.pos='hbtp', out.width='.45\\linewidth', echo=FALSE, error=FALSE, warning=FALSE, message=FALSE>>=
...
ggplot(dat, aes(x = JOB_SUMMARY, y = med_salary)) +
  ...
  coord_flip() +
  labs(x = 'Job Field', y = 'Median Salary', title = 'Median PERM Salary by Field') +
  theme(title = element_text(size = 16),
        axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16))
@

For permanent workers, the medical field dominates in terms of salary, coming in with a median pay of \Sexpr{currency.format(dat$med_salary[dat$JOB_SUMMARY == 'MEDICAL'])}. Lawyers come in at a distant second with a median salary of \Sexpr{currency.format(dat$med_salary[dat$JOB_SUMMARY == 'LAW'])}. 


<<perm-med-wage-by-state, fig.cap='Median wage for PERM workers in Engineering Fields by State', fig.width=8, fig.height=7, fig.align='center', fig.pos='hbtp', out.width='.45\\linewidth', echo=FALSE, error=FALSE, warning=FALSE, message=FALSE>>=
...
When looking geographically at engineering jobs, California, Washington, and New York came through as some of the highest paying states, with median salaries of over \$100,000. 


<<perm-med-wage-school, fig.cap='Median wages for PERM workers by University, showing universities with over 1,000 applications.', fig.width=10, fig.height=7, fig.align='center', fig.pos='hbtp', out.width='.45\\linewidth', echo=FALSE, error=FALSE, warning=FALSE, message=FALSE>>=
# perm count by year
...
p <- ggplot(df.uni, aes(x = FOREIGN_WORKER_INFO_INST, y = med_salary, fill = applications)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
Finally, we examined which universities were most responsible for providing skilled labor by the number of applicants, as well as how well those applicants are paid. Carnegie Mellon University topped the list with alumni making a median salary of over \$110,000. Jawaharlal Nehru Technological University and Anna University however, top the list of applications with over 3600 certified applicants each.
...
<<attorneys, fig.cap='Top H-1B attorney in each state (and D.C.) by number of applications certified.', fig.width=10, fig.height=10, fig.align='center', fig.pos='hbtp', out.width='.45\\linewidth', echo=FALSE, error=FALSE, warning=FALSE, message=FALSE>>=
attorneys <- visas %>%
...
p <- ggplot(attorneys, aes(x=name, y=n)) +
  geom_point(stat="identity") +
  coord_flip() +
...
We analyzed two data sets, LCA applications for the H-1B program, and PERM applications. We replicated a study done on H-1Bs by the Center for Immigration Studies, and were able to conclude that their analysis was correct, but their focus was too narrow and did not generalize well the the H-1B program as a whole. It also neglected to mention that while prevailing wage claims tend to undershoot the median wage, actual H-1B pay is only slightly below the median on average, and in many cases, well above the median. It should be noted though, that H-1B workers may not receive benefits and other compensation seen by citizens or PERM workers. 

It is clear that PERM workers receive 15\% higher wages than H-1B workers on average.

To perhaps no one's surprise, the highest paying jobs can be found in the fields of medicine and law, followed by engineering, management, and finance jobs. Education jobs trailed far behind most other fields.

We found the most successful attorneys for processing H-1B LCA forms in each state. The top attorneys by far were Priscilla Muhlenkamp in Washington, D.C. and Oishika Gupta in Chicago, IL.

We also found that the highest paid graduates tend to come from Carnegie Mellon University. 

\section{Future Work}

We were very ambitious when we started this project. Our end goal was to build a Shiny app that would allow users to filter jobs down to the individual lat-long coordinates and determine where the highest paying jobs are when accounting for cost of living. 

We built a working Shiny app and had collected geographical data for each company in the PERM data set. However, we ran into resource limitations on ShinyApps.io. The data set is large, and even when removing unnecessary columns, the load time for the application exceeded the memory of the free accounts offered on ShinyApps.io. Aggregating the data reduced the usefulness of the app, making it cumbersome and inflexible.

As a future project, it would be ideal to have a paid host that could accommodate the processing requirements of this data. 

In addition, it would be nice to be able to update this report year-over-year as new data comes out. This should be feasible and take minimal effort as long as the format of the data does not change. 
...
\bibitem{NewsRelease}
  ...
  \textit{News Release}, May 19, 2016
  \href{https://www.bls.gov/news.release/archives/forbrn_05192016.pdf}{https://www.bls.gov/news.release/archives/forbrn_05192016.pdf}
 
\bibitem{NumericalLimits}
  Argueta, Carla N.
  Congressional Research Service
  \textit{Numerical Limits on Permanent Employment-Based Immigration: Analysis of the Per-country Ceilings},
  July 28, 2016
  \href{https://fas.org/sgp/crs/homesec/R42048.pdf}{https://fas.org/sgp/crs/homesec/R42048.pdf}
 
\bibitem{AIC} 
  American Immigration Center
  \textit{How Many Immigration Applications Are Filed Each Year?}
\href{https://www.us-immigration.com/how-many-immigration-applications-filed-each-year/}{https://www.us-immigration.com/how-many-immigration-applications-filed-each-year/}

  
\bibitem{MigrationPolicy} 
  Zong, Jie. Batalova, Jeanne 
  Migration Policy Institute
  \textit{Frequently Requested Statistics on Immigrants and Immigration in the United States}, April 14, 2016
  \href{http://www.migrationpolicy.org/article/frequently-requested-statistics-immigrants-and-immigration-united-states}{http://www.migrationpolicy.org/article/frequently-requested-statistics-immigrants-and-immigration-united-states}
...
\bibitem{BLS2015}
  ...
  \href{http://www.bls.gov/oes/current/oes\_nat.htm}{http://www.bls.gov/oes/current/oes\_nat.htm}
...
\bibitem{OFLC2015AR}  
  ...
  \href{https://www.foreignlaborcert.doleta.gov/pdf/OFLC\_Annual\_Report\_FY2015.pdf}{https://www.foreignlaborcert.doleta.gov/pdf/OFLC\_Annual\_Report\_FY2015.pdf}

\bibitem{BLSInflation}
  Bureau of Labor Statistics
  \textit{Inflation Calculator}
  \href{http://www.bls.gov/data/inflation\_calculator.htm}{http://www.bls.gov/data/inflation\_calculator.htm}
